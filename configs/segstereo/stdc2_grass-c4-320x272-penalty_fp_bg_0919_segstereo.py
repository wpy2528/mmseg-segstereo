...
# ===== Data roots（SSD2 已划分：train=segstereo，val=segstereo_val）=====
DATA_ROOT_TRAIN = '/data_SSD2/datasets/segstereo'
DATA_ROOT_VAL = '/data_SSD2/datasets/segstereo_val'
# 若标注已为 0～C-1，可去掉 pipeline 中的 RemapSegLabels
# 须与 CLASS_NAMES 一致：val 中常见 1/50/250；未映射像素会变为 ignore_label
SSD2_LABEL_MAP = {1: 0, 50: 1, 200: 2, 250: 3}
...
# 双目阶段仅有 left_img/right_img，无 img；RandomFlip/依赖 img 的增强须放在 Pack 之后或换成立体专用增强
train_pipeline = [
    dict(type='LoadStereoImages', color_type='color', imdecode_backend='cv2'),
    dict(type='LoadAnnotations'),
    dict(type='RemapSegLabels', mapping=SSD2_LABEL_MAP, ignore_label=255),
