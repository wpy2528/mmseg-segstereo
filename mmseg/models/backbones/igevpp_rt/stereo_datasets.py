...
class StereoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        flow = np.stack([disp, np.zeros_like(disp)], axis=-1) # ? 好像没卵用
...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None, root='sceneflow/', dstype='images', things_test=False):
    ...
    def _add_things(self, split='TRAIN'):
        ...
        root = self.root
        left_images = sorted( glob(osp.join(root, "*things*", '**/left/*.png'), recursive=True) )
    ...
    def _add_monkaa(self, split="TRAIN"):
        ...
        original_length = len(self.disparity_list)
        root = self.root
        left_images = sorted( glob(osp.join(root, "*monkaa*", '**/left/*.png'), recursive=True) )
    ...
    def _add_driving(self, split="TRAIN"):
        ...
        original_length = len(self.disparity_list)
        root = self.root
        left_images = sorted( glob(osp.join(root, "*driving*", '**/left/*.png'), recursive=True) )
        right_images = [ image_file.replace('left', 'right') for image_file in left_images ]
        disparity_images = [ im.replace(self.dstype, 'disparity').replace('.png', '.pfm') for im in left_images ]
...
def fetch_dataloader(args):
    ...
    if args.train_datasets == 'sceneflow':
        ...
        new_dataset = SceneFlowDatasets(aug_params, dstype='images')
