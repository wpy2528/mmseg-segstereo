...
class Slice(SubModule):
    ...
    def forward(self, bilateral_grid, wg, hg, guidemap): 
        ...
        coeff = F.grid_sample(bilateral_grid, guidemap_guide,align_corners =False) # 把B C H W 采样成了B 1 H' W'
...
def build_gwc_volume(refimg_fea, targetimg_fea, maxdisp, num_groups):
    ...
    for i in range(maxdisp):
        if i > 0:            
            ...
            volume[:, :, i, :, :] = groupwise_correlation(refimg_fea, targetimg_fea, num_groups) # 所谓的相关就是两个特征图的点乘取平均 所谓的groupwise就是多头
...
class BGNet_Plus(SubModule):
    ...
    def forward(self, left_right_input):        
        left_input, right_input = torch.split(left_right_input, 1, dim=1)


        left_low_level_features_1,  left_gwc_feature  = self.feature_extraction(left_input) # 特征图是原图的1/8
        _,                          right_gwc_feature = self.feature_extraction(right_input)
        guide = self.guide(left_low_level_features_1) #[B,1,H,W] 引导图是原图的1/2
        cost_volume = build_gwc_volume(left_gwc_feature,right_gwc_feature,25,44) # 构造组相关代价体 44个通道 25个视差值 # ! 相当于MHA

        cost_volume = self.dres0(cost_volume) # 3d卷积
        coeffs = self.coeffs_disparity_predictor(cost_volume)
        # ! 通过插值，把25个视差值插值成97个视差值
        list_coeffs = torch.split(coeffs,1,dim = 1) # 沿着视差轴劈开
        index = torch.arange(0,97)
        index_float = index/4.0
        index_a = torch.floor(index_float) # ! 把插值点划分为上下限两组分别记为a和b
        index_b = index_a + 1
        ...
        # start = time.time()
        for i in range(25):
            slice_dict.append(self.slice(list_coeffs[i], wg, hg, guide)) #[B,1,H,W] 引导滤波 # ! 将代价体从1/8尺寸采样到1/2尺寸
        slice_dict_a = []
        slice_dict_b = []
        for i in range(97):
            inx_a = i//4
        ...
        half_disp = torch.sum(disparity_samples * slice,dim = 1).unsqueeze(1) # 不像SGM一样直接WTA，而是把所有视差值加代价权求和 得到视差图
        ...
        right_half = F.interpolate(
                    ...
                    mode='bilinear',
                    align_corners=False)
        refinement_disp = self.refinement_net(half_disp,left_half,right_half) # 利用原图对视差图进行细化
