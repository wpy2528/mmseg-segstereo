...
def disp_warp(right_input, disparity_samples, padding_mode='border'):
    ...
    right_y_coordinate = left_y_coordinate.expand(
        disparity_samples.size()[1], -1, -1, -1).permute([1, 0, 2, 3]) - disparity_samples # 原坐标-视差图 得到左图像素点在右图中的对应像素点坐标
    ...
    # start  = time.time()
    warped_right_feature_map = torch.gather(right_feature_map, dim=4, index=right_y_coordinate.expand( # 根据左图像素点在右图中的对应像素点坐标，在右图特征图中采样，得到和左图（理论上）一致的右图
    ...
    warped_right_feature_map = (1 - ((right_y_coordinate_1 < 0) +
        ...
        (warped_right_feature_map) + torch.zeros_like(warped_right_feature_map) # 无效位置置0
