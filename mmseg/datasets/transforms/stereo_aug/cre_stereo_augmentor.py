...
class CREStereoAugmentor(BaseTransform):
    ...
    def chromatic_augmentation(self, img):
        ...
        # 修复gamma_map创建方式
        gamma_map = [
            int(255 * 1.0 * pow(ele / 255.0, random_gamma)) for ele in range(256)
        ]
    ...
    def transform(self, results: dict) -> dict:
        ...
        # TODO 加上下边的CREStereoDataset里的flip
        left_img, right_img, left_disp, disp_mask = self.augment(left_img, right_img, left_disp)
        
        results['left_img'] = left_img
        results['right_img'] = right_img
        results['left_disp'] = left_disp
        results['disp_mask'] = disp_mask
        return results

    def augment(self, left_img, right_img, left_disp):
