...
class FlowAugmentor(BaseTransform):
    def __init__(self):
        arg_params = {"crop_size": [320, 768], "min_scale": -0.4, "max_scale": 0.8, "do_flip": False, "yjitter": True, "saturation_range": [0, 1.4]}
        self.initialize(**arg_params)
        
    def initialize(self, crop_size, spatial_scale=True, min_scale=-0.2, max_scale=0.5, do_flip=True, yjitter=False, saturation_range=[0.6,1.4], gamma=[1,1,1,1]):
    ...
    def spatial_transform(self, img1, img2, flow):
        ...
        if self.yjitter:
            ...
            img2 = img2[y1:y1+self.crop_size[0], x0:x0+self.crop_size[1]] # 这个应该是容忍一定的极线未对齐
    ...
    def transform(self, results: dict) -> dict:
        ...
        left_disp = results['left_disp']
        flow = np.stack([left_disp, np.zeros_like(left_disp)], axis=-1)
        valid = left_disp < 1024
        
        left_img, right_img, flow = self.exec(left_img, right_img, flow)
        results['left_img'] = left_img
        results['right_img'] = right_img
        results['left_disp'] = flow[..., 0]
        results['disp_mask'] = valid
        return results

@TRANSFORMS.register_module()
class SparseFlowAugmentor(BaseTransform):
    def __init__(self, crop_size, min_scale=-0.2, max_scale=0.5, do_flip=False, yjitter=False, saturation_range=[0.7,1.3], gamma=[1,1,1,1]):
        raise NotImplementedError("原版里没有用这个")
        # spatial augmentation params
        self.crop_size = crop_size
        self.min_scale = min_scale
        self.max_scale = max_scale
    ...
    def exec(self, img1, img2, flow, valid):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
    ...
    def transform(self, results: dict) -> dict:
        ...
        left_img, right_img, flow, valid = self(left_img, right_img, flow, valid)
        results['left_img'] = left_img
        results['right_img'] = right_img
        results['left_disp'] = flow[0]
        results['disp_mask'] = valid
