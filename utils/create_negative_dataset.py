...
if __name__ == "__main__":
    ...
    parser.add_argument("--is_3x3", action="store_true")
    ...
    if args.is_3x3:
        ...
        src_image_paths = glob.glob(os.path.join(args.src_image_dir, "**", "*.jpg"), recursive=True)
        for src_image_path in tqdm(src_image_paths):
            ...
            dst_image_path = os.path.join(args.dst_dataset_dir, "images", os.path.basename(src_image_path))
            dst_mask_path = os.path.join(args.dst_dataset_dir, "labels", os.path.basename(src_image_path).replace(".jpg", ".png"))
            os.makedirs(os.path.dirname(dst_image_path), exist_ok=True)
            os.makedirs(os.path.dirname(dst_mask_path), exist_ok=True)
            cv2.imwrite(dst_image_path, src_image_np)
            cv2.imwrite(dst_mask_path, src_mask_np)
